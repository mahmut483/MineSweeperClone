...
- `BoardGenerator.GenerateMines()` shifts to the next available cell if a random mine position is already occupied.
- `CellRevealer.Flood()` performs a 4-directional flood-fill for empty cells.
- `BoardRenderer.Draw()` redraws the entire board when state changes.
- `GameManager` sets `QualitySettings.vSyncCount = 1` and `Application.targetFrameRate = 60` for performance consistency.

## Screenshots

Place the screenshot file in the repository root and name it `screenshot.png` or `screenshot.jpg`.
...
- Expand flood-fill to support 8 directions.
- Make the first click always safe.
- Add difficulty levels and a timer/score system.
- Add tilemap animations and sound effects.
- Connect `GameEvents` to a UI HUD for game state feedback.
